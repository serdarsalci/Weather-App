...
class WeatherService {
  ...
  String apiKey = 'appid=6cb7892f881284f744e82a2d1a2db254';
  ...
  void getFiveDailyWeather({
    Function() beforSend,
    Function(FiveDailyWeather fiveDailyWeather) onSuccess,
    Function(dynamic error) onError,
  }) {
    final url = '$baseUrl/forecast?q=$city&lang=en&$apiKey';
    print(url);
    ApiRepository(url: '$url', payload: null).get(
        ...
        onSuccess: (data) => {
              onSuccess(
                  FiveDailyWeather.fromJson(data['list']) ?? List.empty()),
            },
        onError: (error) => {
              print(error),
              onError(error),
